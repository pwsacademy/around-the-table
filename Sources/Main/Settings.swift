...
enum Settings {
     ...
     */
    static let customDomainName = configuration["TEST"] == nil ? settings["customDomainName"] as? String : nil
    
    // Default pictures.
    static let defaultGamePicture = settings["defaultGamePicture"] as! String
    static let defaultGameThumbnail = settings["defaultGameThumbnail"] as! String
    static let defaultProfilePicture = settings["defaultProfilePicture"] as! String
    
    // Used to constrain the results returned by the Google Places autocompletion field.
    static let countries = settings["countries"] as! [String]
    // Used to select the views to render and to localize anything that needs localization (e.g. dates).
    static let locale = settings["locale"] as! String
    // Used to display dates in the correct time zone.
    static let timeZone = TimeZone(identifier: settings["timeZone"] as! String) ?? TimeZone.current
    
    // Used to calculate distances when the user has disabled geolocation.
    enum defaultCoordinates {
        
        static let latitude = settings["defaultCoordinates:latitude"] as! Double
        static let longitude = settings["defaultCoordinates:longitude"] as! Double
    }
    
    enum database {
        
        // The name by which the Compose for MongoDB service is connected to your Bluemix app.
        static let bluemixService = settings["database:bluemixService"] as! String
        // The URI of your MongoDB database when running on localhost.
        static let localURI = settings["database:localURI"] as! String
        // The name of your database. When deploying to a test server, a test database is used.
        static let name = configuration["TEST"] == nil ?
            settings["database:name"] as! String :
